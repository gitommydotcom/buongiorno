...
export async function GET() {
  try {
    ...
    const [events, reminders, weather, traffic, mail] = await Promise.all([
    ...
    // cache narrazione 10 min: se i dati sottostanti cambiano davvero, re-genera
    ...
    const text = await cached(`narration:day:${inputHash}`, 10 * 60, () => complete(system, user, { maxTokens: 600 }));
